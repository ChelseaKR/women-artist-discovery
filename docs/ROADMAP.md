...
- **Re-rank is boost-only with protected unknown slots** (`recommender/rerank.py`): unknown scores and pure-taste ranks stay invariant, while aligned artists reorder only sourced non-unknown slots. The boost remains bounded by `MAX_BOOST` so taste is preserved.
