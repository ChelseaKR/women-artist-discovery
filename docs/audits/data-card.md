...
| **Maintenance** | The cache supplies TTL expiry, and `lavender refresh --user` re-asks upstream about cached artists and reconciles the pending-corrections ledger against observed changes. Without `--user` it replays the demo fixtures and says so. What is *not* shipped is a scheduler, and an upstream retraction is deliberately listed for a human rather than applied — an empty answer is never read as agreement. Citations and fetch dates keep stale rows visible. |
