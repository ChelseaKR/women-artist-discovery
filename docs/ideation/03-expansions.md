...
five minutes.
**Status:** ✅ Implemented on `roadmap/exp-10-methods-writeup-as-a-reproducible`
— `docs/writeup/methods.md` (threat framing, `pipeline/models.py` type-invariant
approach, the boost-only proof via `recommender/exposure.py`'s
`assert_unknown_retained`, §4's exposure numbers, honest limits incl. RR-2),
`recommender/exposure.py` + `tests/test_exposure.py` (the unknown-retention
guarantee checked on emitted output, not just asserted from `rerank.py`'s
construction), `scripts/writeup-check.py` (wired into `make audit`) diffs every
quantitative claim in the writeup against the regenerated
`docs/audits/eval-report.json`.
