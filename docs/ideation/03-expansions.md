...
**Status:** ✅ Implemented on `roadmap/exp-08-multi-provider-playlist-export-li`
— `export/listenbrainz.py` (static-token client posting JSPF to
`/playlist/create`, mirroring `export/spotify.py`), `export/transport.py`
(the `HttpTransport`/`HttpResponse`/`RequestsTransport` trio hoisted out of
`export/spotify.py` and re-exported there for API stability), `export/registry.py`
(`PROVIDERS` mapping both providers behind `PlaylistExport`), `export/tracklist.py`
(`_jspf_document` extracted so `to_jspf` and the ListenBrainz client share one
document builder), `app/dashboard.py` (a second, independent "Connect
ListenBrainz" expander wired to the registry), `docs/audits/privacy-notes.md`
(the ListenBrainz egress row + flow write-up), `tests/test_listenbrainz.py`.
